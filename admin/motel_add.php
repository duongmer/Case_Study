...
$pageTitle   = 'Thêm phòng trọ mới';
$currentPage = 'motel_add';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $latlng      = $conn->real_escape_string(trim($_POST['latlng'] ?? ''));
    $phone       = $conn->real_escape_string(trim($_POST['phone']));
    $user_id     = (int)$_POST['user_id'];
    $category_id = (int)($_POST['category_id'] ?? 1);
    $district_id = (int)$_POST['district_id'];
    $utilities   = $conn->real_escape_string(trim($_POST['utilities'] ?? ''));
    ...
    $conn->query("INSERT INTO motel (title,description,price,area,count_view,address,latlng,images,user_id,category_id,district_id,utilities,phone,approve)
        VALUES ('$title','$description',$price,$area,0,'$address','$latlng','$images',$user_id,$category_id,$district_id,'$utilities','$phone',$approve)");

    header('Location: motel.php?ok=saved'); exit;
}
