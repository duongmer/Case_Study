<?php
  ...
  $sql = "SELECT * FROM motel WHERE approve = 1 ORDER BY created_at DESC";

  if (isset($_GET["tatca"])) {
      $sql = "SELECT * FROM motel WHERE approve = 1 ORDER BY created_at DESC";
  } else {
      if (isset($_GET["xemnn"])) {
          $sql = "SELECT * FROM motel WHERE approve = 1 ORDER BY count_view DESC LIMIT 6";
      } else {
          if (isset($_GET["dangtai"])) {
              $sql = "SELECT * FROM motel WHERE approve = 1 ORDER BY created_at DESC LIMIT 6";
  ...
  <div class="container mx-auto pb-5">
    <div class="d-flex justify-content-between align-items-end mb-4 px-2">
        <h4 class="fw-bold mb-0">Phòng trọ nổi bật</h4>
